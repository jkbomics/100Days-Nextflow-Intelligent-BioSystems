...
workflow.onStart {
    ...
    println "Resume mode: ${workflow.resume}"
    println "Run Name: ${workflow.profile}"
    println "Dynamic resource allocation enabled" 
}
...
workflow {
     ...
     */
    reads = Channel.fromFilePairs("${params.input}/*_{R1,R2}.fastq.gz").collate(params.batch_size)
    /*
     * Process batch-wise
     */
    reads.view {"Processing batch: $it"}
    /*
